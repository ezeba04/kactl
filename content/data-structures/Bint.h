 ...
 * License: CC0
 * Source: User snippet (adapted)
 * Description: Fixed-base big integer for non-negative numbers. Stores digits in base 10**BASEXP
 * using a fixed-capacity array of size LMAX. Supports +, -, *, /, % with ll and with bint,
 * comparisons, and printing. Not intended for negative numbers.
 * Usage:
 *  bint a = 123456789, b("987654321");
 *  bint c = a * b + b;
 *  auto qr1 = ldiv(c, 123);       // divide by ll -> pair<quotient, remainder>
 *  auto qr2 = ldiv(c, a);         // divide by bint -> pair<quotient, remainder>
 *  c.out();                       // prints the number
 * Time: Addition/Subtraction O(L); Multiplication O(L^2); Division O(L^2), where L is number of blocks.
 * Status: tested a bit
 */
#pragma once
